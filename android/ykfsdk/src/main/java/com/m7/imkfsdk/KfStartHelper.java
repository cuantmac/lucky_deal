...
public class KfStartHelper {
    ...
    public void setCustomerJSON(String customerJSON) {
        if(customerJSON != null) {
            edit.putString("customerJSON", customerJSON).apply();
        } else {
            edit.putString("customerJSON", "").apply();
        }
    }
    ...
    private void startKFService() {
        IMChatManager.getInstance().setOnInitListener(new InitListener() {
            ...
            public void onInitFailed(int code) {
                ToastUtils.showShort(context, context.getString(R.string.init_failed, code));
                Log.d("MainActivity", context.getString(R.string.init_failed, code));
                IMChatManager.getInstance().quitSDk();
                loadingDialog.dismiss();
            }
        });
...
//                        ChatActivity.startActivity(context, Constants.TYPE_PEER, peer.getId(), mCardInfo);
    ...
    //配置是否使用前台服务，默认true 使用前台服务
    public  void setUSE_ForegroundService(boolean USE_ForegroundService) {
        IMChatManager.setUSE_ForegroundService(USE_ForegroundService);
    }
}
