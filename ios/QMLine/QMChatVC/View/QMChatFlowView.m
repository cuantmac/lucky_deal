...
- (void)createUI {
    ...
    _pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:QMColor_News_Custom];
...
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = [UIColor colorWithHexString:isDarkStyle ? @"#3D3D3D" : QMColor_News_Agent_Light];
        [self createUI];
    }
    return self;
}
...
- (void)createUI {
    ...
    _label.backgroundColor = [UIColor colorWithHexString:isDarkStyle ? @"#3D3D3D" : @"#F5F6F7"];
    _label.textColor = [UIColor colorWithHexString:isDarkStyle ? @"#D5D5D5" : @"#3B3B3B"];
